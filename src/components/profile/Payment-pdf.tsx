...
import { OrderDetailVm, OrderInformation } from "@/shared";
...
export const PaymentPdf = ({
  ...
  const { id, createdAt, createdBy, totalMoney } = invoice.order;
      ...
      <DialogContent>
        <DialogContent>
          <DocumentPDF
            ...
            id={id}
            orderDetailVm={invoice.orderDetailVm}
          />
        </DialogContent>
        <Box
          noValidate
    ...
    <Tailwind>
      <div>
        ...
        <div>
          ...
          <p className="p-0 mb-1">{createdBy}</p>
        </div>
        ...
        <p className="p-0 leading-5">
          ...
          {new Date(createdAt).toLocaleDateString()}. Payment is due within 15
        ...
        <table className="w-full my-12">
          <thead>
            <tr className="border-b border-gray-300">
              <th className="text-left font-bold py-2">Product Id</th>
              <th className="text-left font-bold py-2">Amount</th>
            </tr>
          </thead>
          <tbody>
            {orderDetailVm.map((item) => {
              return (
                <tr className="border-b border-gray-300" key={item.productId}>
        ...
        <PageBottom>
          <div className="h-px bg-gray-300 my-4" />
          <div className="text-gray-400 text-sm">Invoice #{id}</div>
        </PageBottom>
      </div>
    </Tailwind>
  );
