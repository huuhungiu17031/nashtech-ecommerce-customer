...
const ProductDetail = () => {
  ...
  // @ts-ignore
  const { isLoading: isLoadingProduct, data: productDetail } =
    useGetProductDetail(id);
  const { isLoading: isLoadingImage, data: imageUrlList } =
    useGetProductGalleryVmByProductId(id);
  const { data: userRating } = useGetRatingFromUser(productId);
  const { isLoading: isLoadingAverage, data: averageRating } =
